...
def check_services():
    try:
        executable_dir = get_executable_directory()
        vbs_script_path = os.path.join(executable_dir, "check_services.vbs")
        subprocess.run(["cscript", vbs_script_path], shell=True)
        result_text.config(state=tk.NORMAL)
        result_text.delete("1.0", tk.END)
        result_text.insert(tk.END, "Services checked successfully.")
        result_text.config(state=tk.DISABLED)
    except Exception as e:
        result_text.config(state=tk.NORMAL)
        result_text.delete("1.0", tk.END)
...
services_button = tk.Button(button_frame, text="Check Services", command=check_services, bg=button_bg_color, fg="white", padx=10, pady=5)
services_button.grid(row=0, column=3, padx=10)
