...
def generate_pdf(content):
    ...
    for title, data in content.items():
        ...
        # Check if the data is a dictionary, if so, convert it to a table
        if isinstance(data, dict):
            data_table = create_table(data)
            ...
            if data_rows:
                # Create a table for non-empty data rows
                data_table = create_single_column_table(data_rows)
                elements.append(data_table)
...
def create_single_column_table(data_rows):
    # Define table style for single-column table
    table_style = TableStyle([
        ...
        ('BOTTOMPADDING', (0, 0), (-1, 0), 12),  # Padding for bottom of header row
        ('BACKGROUND', (0, 1), (-1, -1), colors.lightgrey),  # Background color for data rows
        ('GRID', (0, 0), (-1, -1), 1, colors.black),  # Grid lines color
        ('VALIGN', (0, 0), (-1, -1), 'MIDDLE')  # Vertical alignment of all cells
    ])
...
def create_table(data_dict):
    ...
    # Define table style with custom colors
    table_style = TableStyle([
    ...
    try:
        ...
        if str_ipv4:
            if str_ipv4.startswith("10.69."):
                ...
                return "Standalone/Project System"
        else:
